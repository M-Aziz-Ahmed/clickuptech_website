    ...
    <header className="font-[Proxima] header fixed shadow-sm tracking-wide z-50 mt-3 mx-3 rounded-lg text-lg  ">
      <section className="flex items-center justify-between gap-4 py-3 px-4 lg:px-10 ">
        ...
        <div className="hidden lg:flex items-center gap-8 ml-auto mr-10">
          ...
          <Link
            href="/institude"
            className=" font-bold  text-white  bg-gradient-to-r  from-red-400 to-yellow-500 px-4 py-1 rounded-lg transition-all duration-300 hover:from-[#fe8c00] hover:to-[#f83600] animate-pulse hover:animate-none"
          ...
          <nav className="flex flex-col gap-4">
            {navItems.map((item) => (
              <div key={item.label} className="relative">
                <Link
                  href={item.link || "#"}
                  className="flex items-center text-white hover:text-[#54B23D]"
                >
                  {item.label}
                  {(item.label === "Services" ||
                    item.label === "Hire Developers") && (
                  ...
                  <ul className="absolute top-[80%] left-0 w-60 bg-white shadow-lg rounded-lg p-2 transition-all duration-300 opacity-0 invisible group-hover:opacity-100 group-hover:visible">
                    {item.children.map((child) => (
                      <li key={child.label}>
                        <Link
                          href={child.link || "#"}
                          className="block text-sm text-white hover:text-[#54B23D]"
                        >
                          {child.label}
                        </Link>
                      </li>
